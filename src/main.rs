...
fn generate_stars(count: usize) -> Vec<Vector3> {
    ...
    for _ in 0..count {
        // Generar un punto aleatorio en una esfera unitaria
...
let sun_base_pos = Vector3::new(0.0, 0.0, 0.0);
    ...
    let obj = Obj::load("models/sphere.obj").expect("Error al leer archivo"); 
...
let stars = generate_stars(2000);
        ...
        for star in &stars { 
            ...
            if proj_pos.w > 0.0 {
                ...
                if ndc_x >= -1.0 && ndc_x <= 1.0 && ndc_y >= -1.0 && ndc_y <= 1.0 {
                    ...
                    let screen_y = (1.0 - ndc_y) * 0.5 * window_height as f32; 
    ...
    let uniforms_ship = Uniforms {
        ...
        view_matrix: static_view_matrix, 
        projection_matrix,               
        viewport_matrix,
        time: elapsed,
    };
